...
sudo apt-get -y install subversion imagemagick libdbus-1-dev libdbus-glib-1-dev libfontconfig1-dev libfreetype6-dev libfribidi-dev libimlib2-dev librsvg2-bin libspeechd-dev libxml2-dev ttf-liberation libgtk2.0-dev

# This is for the compiling process:
sudo apt-get -y install gcc g++ cmake make zlib1g-dev libpng12-dev librsvg2-bin

# This is for the SDL graphics (suggested):
sudo apt-get -y install libsdl-image1.2-dev libdevil-dev libglc-dev freeglut3-dev libxmu-dev libfribidi-dev

# This is for the OpenGL support:
sudo apt-get -y install libglc-dev freeglut3-dev libgl1-mesa-dev libfreeimage-dev
...
#Do 
#sudo apt-get install gpsd gpsd-clients python-gps Then sudo gpsd /dev/ttyAMA0 -F /var/run/gpsd.sock test with cpgs -s to autostart gpsd type sudo dpkg-rconfigure gpsd select yes add /dev/ttyAMA0 defaults for everything else.
