#!/usr/bin/env python
# Light Reich - a Steve Reich like composer
# For light dependant resistors on the Raspberry Ripple
...
def main():
 ...
 print "Hi from Python :- LDR Steve Reich machine"
 # define the thresholds
 runThreshold = 64 # run / stop threshold value
 sensitivity = [1,8,8,2] # value to multiply each of the readings by
      ...
      for s in range (0,8): # play each note in the sequence
        for c in range (0,4): # read in all the controls
            ...
            if abs(temp - cont[c]) >1:
               cont[c] = temp
               drawControls(c,temp)
        if cont[3]>runThreshold: # do not play notes if not running
          tone[seq1[s]].play()
        sleep(cont[0]/512.0)
   if cont[3] > runThreshold: # skip mutate if not running  
