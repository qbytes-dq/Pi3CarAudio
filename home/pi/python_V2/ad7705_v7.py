...
CHN_COMM = 0x2 #common; calibration register pair 0 #AD7706 only
CHN_AIN3 = 0x3 #AIN3; calibration register pair 2   #AD7706 only
...
#    def initChannel(self,channel,clkDivider=CLK_DIV_1,polarity=BIPOLAR,gain=GAIN_1,updRate=UPDATE_RATE_20) :
    def initChannel(self,channel,clkDivider=CLK_DIV_1,polarity=BIPOLAR,gain=GAIN_1,updRate=UPDATE_RATE_60) :
#    def initChannel(self,channel,clkDivider=CLK_DIV_1,polarity=BIPOLAR,gain=GAIN_2,updRate=UPDATE_RATE_25) :
#    def initChannel(self,channel,clkDivider=CLK_DIV_1,polarity=BIPOLAR,gain=GAIN_4,updRate=UPDATE_RATE_25) :
    ...
    def readVoltage(self,channel,vref,factor=1) :    
        return (float(self.readADResultRaw(channel)) / 65535.0 * vref  * factor)
...
#    vref = 2.511
    ...
    while True :
        v1 = ad7705.readVoltage(CHN_AIN1,vref,fact)
        time.sleep (delay)
        v2 = ad7705.readVoltage(CHN_AIN2,vref,fact)

        print ("%s  V1 %.4f  V2 %.4f" % (datetime.now(), v1,  v2))
        time.sleep (delay)
